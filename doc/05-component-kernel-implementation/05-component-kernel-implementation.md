...
I chose to represent PlaylistQueue using a Sequence<String> because it directly supports the required operations. Adding a track to the end, removing the next track, and previewing the next track can all be implemented using sequence operations.

By convention, rep is never null and the front of the queue is stored at position 0.

By correspondence, the value of this is exactly the sequence stored in rep in the same order.
